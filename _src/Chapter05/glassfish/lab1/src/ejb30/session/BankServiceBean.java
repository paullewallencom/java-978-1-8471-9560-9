...
public class BankServiceBean implements BankService {
   ...
   private EntityManager em;
    ...
    public List<Object[]> simpleQuery2() { 
        return (List<Object[]>)em.createQuery("SELECT c.firstName, c.lastName FROM Customer c").getResultList();
    }

    public List<String> simpleQuery3() { 
        return (List<String>)em.createQuery("SELECT DISTINCT c.lastName FROM Customer c").getResultList();
    }

    public List<Customer> simpleQuery4() { 
        return (List<Customer>)em.createQuery("SELECT c FROM Customer c WHERE c.lastName = 'PAGE' ").getResultList();
    }

    public List<Customer> simpleQuery5() { 
        return (List<Customer>)em.createQuery(
        "SELECT c FROM Customer c WHERE c.lastName = 'KING' AND c.firstName = 'SIMON' ").getResultList();
    }

    public List<Address> simpleQuery6() { 
        return (List<Address>)em.createQuery("SELECT ad FROM Address ad WHERE ad.postCode IS NULL").getResultList();
    }

    public List<Account> simpleQuery7() { 
        return (List<Account>)em.createQuery("SELECT ac FROM Account ac WHERE ac.balance BETWEEN 0 AND 100").getResultList();
    }

    public List<Account> simpleQuery8() { 
        return (List<Account>)em.createQuery("SELECT ac FROM Account ac WHERE ac.id IN (1,2)").getResultList();
    }

    public List<Account> simpleQuery9() { 
        return (List<Account>)em.createQuery("SELECT ac FROM Account ac WHERE ac.id = 1 OR ac.id = 2").getResultList();
    }

    public List<Customer> simpleQuery10() { 
        return (List<Customer>)em.createQuery("SELECT c FROM Customer c WHERE c.firstName LIKE '_A%E%' ").getResultList();
    }

    public List<Account> simpleQuery11() { 
        return (List<Account>)em.createQuery("SELECT ac FROM Account ac ORDER BY ac.balance").getResultList();
    }

    public List<Account> simpleQuery12() { 
        return (List<Account>)em.createQuery("SELECT ac FROM Account ac ORDER BY ac.balance DESC").getResultList();
    }

    public List<Account> simpleQuery13() {
        return (List<Account>)em.createQuery("SELECT ac FROM Account ac ORDER BY ac.accountType ASC, ac.balance DESC")
        .getResultList();
    }
